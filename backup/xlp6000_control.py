...
def update_metrics(n):
    ...
    # Clean left-to-right P&ID diagram
    pid_diagram = html.Div([
        # Buffer vessels row
        html.Div([
            html.Span("Buffers:", style={'fontWeight': '600', 'marginRight': '10px', 'fontSize': '12px'}),
            *[html.Span(f"A{i+1}", style={
                'backgroundColor': '#3b82f6' if f'A{i+1}' == current_inlet else '#e5e7eb',
                'color': 'white' if f'A{i+1}' == current_inlet else '#374151',
                'padding': '4px 8px', 'borderRadius': '12px', 'fontSize': '10px',
                'fontWeight': '600', 'marginRight': '6px', 'display': 'inline-block'
            }) for i in range(9)]
        ...
        html.Div([
            # Flow line
            html.Div(style={
                'height': '3px', 'backgroundColor': '#3b82f6' if is_flow_active else '#9ca3af',
                'flex': '1', 'margin': '20px 0'
            }),
            
            # System components in a clean horizontal line
            html.Div([
                # XLP 6000 Pump
                html.Div([
                    html.Div("XLP 6000", style={
                        'backgroundColor': '#1e40af' if is_pump_active else '#64748b',
                        'color': 'white', 'padding': '12px 16px', 'borderRadius': '6px',
                        'fontWeight': '600', 'fontSize': '11px', 'textAlign': 'center'
                    }),
                    html.Div(f"{controller.current_position} steps", style={
                        'fontSize': '9px', 'color': '#6b7280', 'textAlign': 'center', 'marginTop': '4px'
                    })
                ]),
                
                html.Div("→", style={'fontSize': '18px', 'margin': '0 20px', 'color': '#3b82f6' if is_flow_active else '#9ca3af'}),
                
                # Column
                html.Div([
                    html.Div("COLUMN", style={
                ...
                # 3-Way Valve with outputs
                html.Div([
                    html.Div("3-WAY VALVE", style={
                    ...
                    html.Div([
                        html.Div([
                            html.Div("WASTE", style={
                                'backgroundColor': '#ef4444' if controller.valve_position == 'Bypass' else '#fca5a5',
                                'color': 'white', 'padding': '6px 10px', 'borderRadius': '4px',
                                'fontSize': '9px', 'fontWeight': '600'
                            })
                        ], style={'margin': '8px 4px 0 0'}),
                        html.Div([
                            html.Div("COLLECT", style={
                                'backgroundColor': '#10b981' if controller.valve_position == 'Output' else '#86efac',
                                'color': 'white', 'padding': '6px 10px', 'borderRadius': '4px',
                                'fontSize': '9px', 'fontWeight': '600'
                            })
        ...
        # Status row
        html.Div([
            html.Div(f"Pressure: {controller.data_buffer['pressure'][-1]:.1f} psi" if controller.data_buffer['pressure'] else "Pressure: 0.0 psi", 
                    style={'fontSize': '10px', 'margin': '0 15px', 'color': '#6b7280'}),
            html.Div(f"Flow: {controller.current_flow_rate:.2f} mL/min", 
                    style={'fontSize': '10px', 'margin': '0 15px', 'color': '#6b7280'}),
            html.Div(f"Mode: {controller.operation_state.title()}", 
                    style={'fontSize': '10px', 'margin': '0 15px', 'color': '#6b7280'})
...
def update_manual_pid(inlet_valve, outlet_valve):
    ...
    # Clean left-to-right manual P&ID diagram
    pid_diagram = html.Div([
        # Inlet valves selection
        html.Div([
            html.Span("Inlet:", style={'fontWeight': '600', 'marginRight': '10px', 'fontSize': '12px'}),
            *[html.Span(f"A{i+1}", style={
                'backgroundColor': '#10b981' if f'A{i+1}' == inlet_valve else '#e5e7eb',
                'color': 'white' if f'A{i+1}' == inlet_valve else '#374151',
                'padding': '4px 8px', 'borderRadius': '12px', 'fontSize': '10px',
                'fontWeight': '600', 'marginRight': '6px', 'display': 'inline-block',
                'cursor': 'pointer'
            }) for i in range(6)]
        ...
        html.Div([
            ...
            html.Div([
                ...
                html.Div("→", style={'fontSize': '18px', 'margin': '0 20px', 'color': '#374151'}),
                ...
                html.Div([
                    ...
                    html.Div([
                        html.Div("WASTE", style={
                            'backgroundColor': '#ef4444' if outlet_valve == 'Waste' else '#fca5a5',
                            'color': 'white', 'padding': '6px 10px', 'borderRadius': '4px',
                            'fontSize': '9px', 'fontWeight': '600', 'margin': '8px 4px 0 0'
                        }),
                        html.Div("COLLECT", style={
                            'backgroundColor': '#10b981' if outlet_valve == 'Collect' else '#86efac',
                            'color': 'white', 'padding': '6px 10px', 'borderRadius': '4px',
                            'fontSize': '9px', 'fontWeight': '600', 'margin': '8px 0 0 4px'
                        })
                    ], style={'display': 'flex', 'justifyContent': 'center'})
                ])
            ], style={'display': 'flex', 'alignItems': 'center', 'justifyContent': 'center'})
